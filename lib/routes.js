...
const { workstreams, workstreamFromRepo } = require("./helpers/workstreams.js");
...
module.exports = [
  ...
  {
    // We could also fold this into the hook below if we trust things enough.
    path: "/push-status-check",
    method: "POST",
    async handler(ctx) {
      if (validateGitHubWebhook(ctx, workstreamFromRepo) === "pull_request") {
  ...
  },
  {
    ...
    method: "POST",
    async handler(ctx) {
      if (validateGitHubWebhook(ctx, workstreamFromRepo) === "push") {
